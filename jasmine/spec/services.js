        ...
        describe('WORKER service type', function () {
            ...
            describe('dedicated worker', function () {
                ...
                beforeAll(function () {
                    ...
                    services.register('worker', {
                        type: services.ServiceType.WORKER,
                        worker: worker
                    });
                });

                it('should verify service type is working with dedicated worker', function (done) {
                    services.send('worker', {
                        message: 'Hello worker!'
                    }).then(function (response) {
                        expect(response.message).toEqual('Hello worker! Hello main!');
                        done();
                    }, function (error) {
                        fail('because onError is called');
                        done();
                    });
                    services.flush('worker');
                });

                afterAll(function () {
                    services.unregister('worker');
                    worker.terminate();
            ...
            describe('shared worker', function () {
                ...
                beforeAll(function () {
                    ...
                    services.register('shared-worker', {
                        type: services.ServiceType.WORKER,
                        worker: sharedWorker.port
                    });
                });

                it('should verify service type is working with shared worker', function (done) {
                    services.send('shared-worker', {
                        message: 'Hello shared worker!'
                    }).then(function (response) {
                        expect(response.message).toEqual('Hello shared worker! Hello main!');
                        done();
                    }, function (error) {
                        fail('because onError is called');
                        done();
                    });
                    services.flush('shared-worker');
                });

                afterAll(function () {
                    services.unregister('shared-worker');
